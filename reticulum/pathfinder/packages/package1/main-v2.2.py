...
def intermediate_node(configpath):
    ...
    identity = RNS.Identity.from_file("/opt/rns_pathfinder/package1/identity")
...
def query_handler(message, packet):
    ...
    if isinstance(payload, dict):
        ...
        if find_path_has_path(target_hash):
            next_hop = RNS.Transport.next_hop(target_hash)
            node_id = next(open("/opt/rns_pathfinder/package1/id.txt")).strip()
            return_payload = {
                "id": node_id,
                "nh_ihh": next_hop.hex(),
            }
            # recalling identity doesn't seem to work
            sender_identity = RNS.Identity.from_file("/opt/rns_pathfinder/package1/startIdentity")
